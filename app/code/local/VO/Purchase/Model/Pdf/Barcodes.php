 ...
 *
 */
abstract class VO_Purchase_Model_Pdf_Barcodes extends VO_Purchase_Model_Pdf_Pdfhelper
{
	public function drawUPC(&$page, $x, $y, $code,$width = null,$height = null,$type = 'UPC-A')
	{
		if (strtoupper($type) == 'UPC-A' && strlen($code) == 12)
		{
			...
			//Note in the left hand 0 means black
			...
			//Convert everything to arrays of single digits in serial
			//i.e. array(2,3,1,5,6,7,2,3,4,1,5,6,7,2)
			$left = str_split($left);
			...
			{
				//module is a series of binary bars representing one decimal digit, the value of the left or right array is used to access it's binary
				//representation from the index (whose keys are the decimal values).
				$module = str_split($index[$digit]);

				//now we have an array of each bar that needs to be written for this single base 10 number ($left[$key])
